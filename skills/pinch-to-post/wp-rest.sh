...
# set -e  # Disabled - was causing silent failures
...
# Normalize URL - remove trailing /wp-json if present (we add it in commands)
WP_SITE_URL="${WP_SITE_URL%/wp-json}"
...
markdown_to_gutenberg() {
    ...
    while IFS= read -r line || [ -n "$line" ]; do
        ...
        if [[ "$line" =~ ^\<(table|thead|tbody|tr|td|th) ]]; then
            in_html_block=true
            html+="${line}\n"
            continue
        elif [[ "$line" =~ ^\<\/(table|thead|tbody|tr|td|th) ]]; then
            html+="${line}\n"
            in_html_block=false
            continue
        elif $in_html_block; then
            html+="${line}\n"
            continue
        fi
        ...
        if [[ "$line" =~ ^###\ (.+)$ ]]; then
            heading_text=$(convert_inline "${BASH_REMATCH[1]}")
            ...
            quote_text=$(convert_inline "${BASH_REMATCH[1]}")
            html+="<!-- wp:quote -->\n<blockquote class=\"wp-block-quote\"><p>${quote_text}</p></blockquote>\n<!-- /wp:quote -->\n\n"
        elif [[ "$line" =~ ^[*-]\ (.+)$ ]]; then
            in_list=true
            item_text=$(convert_inline "${BASH_REMATCH[1]}")
...
case "$1" in
    ...
    update-post-markdown)
        ...
        POST_ID="$2"
        MD_FILE="$3"
        [ -z "$POST_ID" ] || [ -z "$MD_FILE" ] && { echo "Usage: update-post-markdown <post_id> file.md" >&2; exit 1; }
        ...
        RESPONSE=$(wp_curl -X POST "${WP_SITE_URL}/wp-json/wp/v2/posts/${POST_ID}" \
            -H "Content-Type: application/json" \
    ...
    stats)
        ...
        for status in publish draft pending future; do
            COUNT=$(wp_curl "${WP_SITE_URL}/wp/v2/posts?status=${status}&per_page=1" -I 2>/dev/null | \
                grep -i 'x-wp-total:' | awk -F': ' '{print $2}' | tr -d '\r')
            echo "Posts (${status}): ${COUNT:-0}"
        done
        PAGES=$(wp_curl "${WP_SITE_URL}/wp/v2/pages?per_page=1" -I 2>/dev/null | \
            grep -i 'x-wp-total:' | awk -F': ' '{print $2}' | tr -d '\r')
        echo "Pages: ${PAGES:-0}"
        MEDIA=$(wp_curl "${WP_SITE_URL}/wp/v2/media?per_page=1" -I 2>/dev/null | \
            grep -i 'x-wp-total:' | awk -F': ' '{print $2}' | tr -d '\r')
        echo "Media: ${MEDIA:-0}"
        COMMENTS=$(wp_curl "${WP_SITE_URL}/wp/v2/comments?per_page=1" -I 2>/dev/null | \
            grep -i 'x-wp-total:' | awk -F': ' '{print $2}' | tr -d '\r')
...
EOF
    ...
    site-health)
        ...
        echo "=== Site Health ==="
        echo -n "REST API: "
        wp_curl_public "${WP_SITE_URL}/" | jq -e '.name' > /dev/null 2>&1 && echo "✅ OK" || echo "❌ Failed"
        echo -n "Auth: "
        wp_curl "${WP_SITE_URL}/wp/v2/posts?per_page=1" | jq -e '.[0].id' > /dev/null 2>&1 && echo "✅ OK" || echo "❌ Failed"
        echo -n "Response: "
        TIME=$(curl -s -o /dev/null -w "%{time_total}" "${WP_SITE_URL%/wp-json}")
        echo "${TIME}s"
        ;;
...
POSTS:
  ...
  update-post-markdown <id> file.md
...
EXAMPLES:
  ...
  wp-rest.sh update-post-markdown 123 article.md
