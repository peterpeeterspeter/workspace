#!/usr/bin/env python3
"""Extract Week 1 Day 1 keyword research results - FINAL VERSION"""
...
for diff_file in difficulty_files:
    ...
    if diff_path.exists():
        ...
        try:
            tasks = diff_data.get("data", {}).get("tasks", [])
            for task in tasks:
                ...
                for result_item in result:
                    if "items" in result_item:
                        ...
                        for item in items:
                            kw = item.get("keyword", "")
                            diff = item.get("keyword_difficulty", None)
                            if kw:
                                difficulty_scores[kw] = diff
        except Exception as e:
            print(f"Error reading difficulty from {diff_file}: {e}")
...
for ov_file in overview_files:
    ...
    try:
        tasks = ov_data.get("data", {}).get("tasks", [])
        for task in tasks:
            result = task.get("result", [])
            for result_item in result:
                if "items" in result_item:
        ...
        print(f"Error reading overview from {ov_file}: {e}")

keywords = list(keywords_data.values())
