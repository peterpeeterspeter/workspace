...
BATCH_SIZE = 5         # Test batch: only first 5 posts
...
def main():
    ...
    print(f"🧪 TEST BATCH: Generating {len(posts)} images")
    ...
    print(f"   Full run (49): ${49 * cost_per_image:.2f}")
    print()
    ...
    # Save test results
    output_file = INPUT_FILE.replace('.json', '-test-batch.json')
    with open(output_file, 'w') as f:
        json.dump(posts, f, indent=2)

    # Summary
    print("=" * 50)
    print(f"✅ Test batch complete: {success_count}/{len(posts)} images")
    ...
    print(f"💰 Test batch cost: ${actual_cost:.2f}")
    print(f"   Full run (49) would cost: ${49 * 0.05:.2f}")
    ...
    if failed_posts:
        ...
        print()
        print("Next steps:")
        print("1. Review generated images in: temp/images/")
